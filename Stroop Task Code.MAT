function trials = stroop_2020(num, OutFile,lang)%OutFile is the output file name
%English STROOP: The Stroop test
%
% SYNOPSIS
% trials = stroop();
% trials = stroop(num);
...
%
% User input
if nargin<=2, lang = 'en'; end;%Changed this to 2 because we now have the OutFile argument to specify.
if nargin<=0, num = 100; end;
% Load language
eval(['lang_' lang]);
WORDS = {'?erven?', 'modr?', 'zelen?', 'fialov?'};
...
'ToolBar', 'none');
% Display instruction
ht = show_text(h, lang.instructions, 'FontSize', 30);
waitforspace(h);
delete(ht);
% Session variables
